...
int main ( int argc, char * argv[] ) {
  ...
  FloatT *CV, *CE, *MU, *RE, *FE, *VR, *VF, *VRmax, *VFmax, *RMax, *FMax;
  ...
  auto cdist_node = [=] __device__(Int const& offset) {
    IntT i = offset / patt.num_nodes;
    IntT j = offset % patt.num_nodes;
  ...
  auto cdist_edge = [=] __device__(Int const& offset) {
    IntT i = offset / patt.num_edges;
    IntT j = offset % patt.num_edges;
  ...
  for (Int i = 0; i < patt.num_nodes; i++) {
    ...
    auto RepeatColumnsByDataEdges_op = [=] __device__(Int const& offset) {
      ...
      Int src    = data.srcs[ij];
      FE[offset] = - CE[offset] + VR[src * patt.num_edges + km];
      RE[offset] = - CE[offset] + VF[src * patt.num_edges + km];
    };
    thrust::for_each(
      thrust::device,
      thrust::make_counting_iterator<Int>(0),
